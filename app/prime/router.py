...
PROBLEMS = {
    ...
    "geometry": [
        ...
        {"question": "Hypotenuse of a right triangle with legs 3 and 4?", "answer": 5.0, "hint": "Use the Pythagorean theorem: a²+b²=c²", "difficulty": 2},
...
# ── In-memory session state (replace with DB later) ──────────────────────────
student_state: dict = {}
...
def submit_answer(payload: AnswerSubmit):
    # Get current difficulty for this student
    state = student_state.get(payload.student_id, {"difficulty": 1, "streak": 0})
    
    # Find the problem answer (simplified — use DB in production)
    all_problems = [p for bank in PROBLEMS.values() for p in bank]
    correct_answer = 4.0  # fallback
    for p in all_problems:
        if abs(p["answer"] - payload.answer) < 0.1:
