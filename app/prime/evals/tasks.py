...
TASKS: List[EvalTask] = [
    ...
    EvalTask(
        ...
        checks=[r"(?i)(CORS|middleware|not registered|allow_methods|not allowed|only accepts|only.*POST|POST.*only|expects POST|use            	POST|expects.*POST|configured to accept)"],
        engineer_mode=True,
    ...
    EvalTask(
        ...
        checks=[r"(?i)(ALTER TABLE|rename.*column|update.*model|one fix|not both|mismatch|kind.*entry_type|entry_type.*kind|doesn.t 	exist|does not exist|SQLAlchemy.*fail|fail.*SQLAlchemy)"],
        engineer_mode=True,
